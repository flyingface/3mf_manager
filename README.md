...
| **作品分组** | 把相关文件（同一模型的部件/变体/重复导出/配件）组成「作品」：可 AI 扫描建议建组，也可手动「＋ 新建分组」搜索勾选；成员可逐个设置组内角色（★ 主文件每组唯一，组件/变体/配件/重复/其他不限）、添加/移出、重命名、解散分组；只记录关系层，不移动不修改文件本体 |
...
| POST | `/api/groups` | 作品分组：列表 / `create` 建组 / `update` 改名换封面 / `delete` 解散 / `member` 成员增删、角色、主文件、打印状态 |
